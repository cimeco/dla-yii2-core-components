...
class Menu extends Widget
{
     ...
     */
    public $linkTemplate = '<td class="{colorClass}"><a href="{url}">{label}</a></td>';
    /**
     * @var string the template used to render the body of a menu which is NOT a link.
     * In this template, the token `{label}` will be replaced with the label of the menu item.
     * This property will be overridden by the `template` option set in individual menu items via [[items]].
     */
    public $labelTemplate = '<td class="{colorClass}">{label}</td>';
    /**
    ...
    public function run()
    {
        ...
        Yii::$app->getView()->registerCss($styles);
    }
    ...
    protected function renderItems($items, $parent = null)
    {
        ...
        foreach ($items as $i => $item) {
            $options = array_merge($this->itemOptions, ArrayHelper::getValue($item, 'options', ['colspan'=>2]));
            $tag = ArrayHelper::remove($options, 'tag', 'tr');
            
            if(!$parent){
                $item['colorClass'] = $this->itemClassBase.$i;
                $this->styles[ $item['colorClass'] ] = ArrayHelper::remove($options, 'background-color', '#444');
            }else{
                $item['colorClass'] = $parent;
            }
            ...
            if (!empty($item['items'])) {
                ...
                $subitems_chunck = array_chunk($item['items'], 2);
                
                foreach($subitems_chunck as $subitems){
                
                    $menu .= '<tr>';
                    
                    $submenuTemplate = ArrayHelper::getValue($item, 'submenuTemplate', $this->submenuTemplate);
                    $menu .= strtr($submenuTemplate, [
                        '{items}' => $this->renderItems($subitems, $item['colorClass']),
                    ]);
    ...
    private function makeStyles(){
        ...
        foreach($this->styles as $class=>$style){
            $styles .= ".$class{ background-color: $style; }";
        }
